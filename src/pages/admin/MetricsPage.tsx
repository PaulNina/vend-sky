...
export default function MetricsPage() {
  ...
  const loadMetrics = async () => {
    ...
    if (!sales) { setWeeklyData([]); setCityData([]); setLoading(false); return; }
    ...
    <div className="space-y-6 max-w-6xl">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-0.5">Desglose semanal y por ciudad</p>
        </div>
        <Select value={selectedCampaign} onValueChange={setSelectedCampaign}>
          <SelectTrigger className="w-[260px]"><SelectValue placeholder="Campaña" /></SelectTrigger>
          <SelectContent>
            ...
            <Card>
              <CardContent className="p-0">
                <Table>
                  ...
                  <TableBody>
                    {cityData.length === 0 ? (
                      <TableRow><TableCell colSpan={6} className="text-center text-muted-foreground py-12">Sin datos para esta campaña</TableCell></TableRow>
                    ) : cityData.map((c) => (
                      <TableRow key={c.city}>
