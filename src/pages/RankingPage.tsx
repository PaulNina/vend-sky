    ...
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          ...
          <p className="text-xs sm:text-sm text-muted-foreground mt-0.5">Clasificación por puntos</p>
        </div>
      ...
      {loading ? (
        ...
        <Tabs defaultValue="general">
          ...
          <TabsContent value="general" className="space-y-4 mt-4">
            {/* City filter chips - scrollable on mobile */}
            <ScrollArea className="w-full">
              <div className="flex gap-1.5 pb-2">
                {cities.map((city) => (
                  <Badge
                    ...
                    className="cursor-pointer text-[10px] sm:text-[11px] transition-colors whitespace-nowrap shrink-0"
              ...
              {top3.map((entry, i) => {
                ...
                return (
                  <Card key={entry.vendor_id} className={`border-2 ${p.bg} hover:shadow-lg transition-shadow`}>
                    <CardContent className="p-5 sm:p-6 text-center space-y-2 sm:space-y-3">
                      <p.icon className={`h-10 w-10 sm:h-12 sm:w-12 mx-auto ${p.color}`} />
                      <p className="text-2xl sm:text-3xl font-bold font-display">#{i + 1}</p>
                      <p className="text-lg sm:text-xl font-semibold">{entry.full_name}</p>
                      <p className="text-xs sm:text-sm text-muted-foreground">{entry.store_name}</p>
                      <Badge variant="outline" className="text-[11px]">{entry.city}</Badge>
                      <div className="grid grid-cols-3 gap-2 pt-2">
                        <div>
                          <p className="text-xl sm:text-2xl font-bold text-primary font-display">{entry.total_points}</p>
                          <p className="text-[9px] sm:text-[10px] text-muted-foreground uppercase tracking-widest">Puntos</p>
                        </div>
                        <div>
                          <p className="text-xl sm:text-2xl font-bold font-display">Bs {entry.total_bonus_bs}</p>
                          <p className="text-[9px] sm:text-[10px] text-muted-foreground uppercase tracking-widest">Bono</p>
                        </div>
                        <div>
                          <p className="text-xl sm:text-2xl font-bold font-display">{entry.total_units}</p>
                          <p className="text-[9px] sm:text-[10px] text-muted-foreground uppercase tracking-widest">Uds</p>
                        </div>
                      </div>
                    </CardContent>
                  </Card>
          ...
          <TabsContent value="ciudad" className="mt-4 space-y-4 sm:space-y-6">
